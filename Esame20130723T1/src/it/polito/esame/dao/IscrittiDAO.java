...
import it.polito.esame.model.StudenteIdMap;
...
public class IscrittiDAO {
	...
	public static void main(String[] args) {
		...
		//dao.getStudenteByMatricola(146101);
		
		
	}

	public Studente getStudenteByMatricola(int matricola, StudenteIdMap studenteIdMap) {
		...
		try {
			...
			if (rs.next()) {
				...
				studenteIdMap.put(s, s.getMatricola());
			}
	...
	public List<Studente> getStudentiSimili(Studente s, StudenteIdMap studenteIdMap) {
		final String sql = "SELECT  DISTINCT s.matricola, s.cognome, s.nome, i1.codins " + 
				"from studente as s, iscrizione as i1, iscrizione as i2, iscrizione as i3 " + 
				"WHERE s.matricola = i1.matricola AND " + 
				"		i1.codins = i2.codins AND " + 
				"		i2.matricola = i3.matricola AND " + 
				"		i3.matricola = ? ";
		...
		try {
			...
			while (rs.next()) {
				...
				s = new Studente(rs.getInt("matricola"), rs.getString("cognome"), rs.getString("nome"), null);
				
				//result.add(studenteIdMap.get(s));
				
				//result.get(result.size()-1).setNcorsi(result.get(result.size()-1).getNcorsi()+1);
				
				studenteIdMap.get(s).setNcorsi(studenteIdMap.get(s).getNcorsi()+1);
